...
with open("label_comment_topics.yaml", "r") as file:
...
final_text_topic_df["comment_id"] = df["comment_id"]
final_text_topic_df.to_csv("../output/comments_topics.csv", index=False)

final_topic_info_df = pd.concat(all_topic_info_dfs, ignore_index=True)
final_topic_info_df.to_csv("../output/comments_topic_info.csv", index=False)
