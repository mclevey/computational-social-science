...
def plot_distance_per_day(data, time_col, distance_col, num_segments=15, filename=None):
    ...
    # Create color map for the gradient (Cool-Warm, where red is closer, blue is further)
    cmap = plt.cm.coolwarm.reversed()

    # Normalize the color map
    norm = plt.Normalize(vmin=data[distance_col].min(), vmax=data[distance_col].max())
    ...
    fig, axes = plt.subplots(
        nrows=num_days, ncols=1, figsize=(12, 4 * num_days), layout="none"
    )
    # fig.subplots_adjust(hspace=0.4, top=0.01, bottom=0.03)
    # Add space between subplots, change top and bottom margins
    ...
    for i, day in enumerate(unique_days):
        ...
        day_data["hours_elapsed"] = (
            day_data[time_col] - pd.to_datetime(day)
        ).dt.total_seconds() / 3600

        # Plot the distance as a gradient line
        for j in range(1, len(day_data)):
            x_vals = [
                day_data["hours_elapsed"].iloc[j - 1],
                day_data["hours_elapsed"].iloc[j],
            ]
            y_vals = [
                day_data[distance_col].iloc[j - 1],
                day_data[distance_col].iloc[j],
            ]
            ...
            lc = mcolors.LinearSegmentedColormap.from_list(
                "segment_gradient", [color_start, color_end]
            )

            # Adjust the number of segments between two points
            # to create a smoother or rougher transition
            for k in range(num_segments):
                segment_start_x = x_vals[0] + (x_vals[1] - x_vals[0]) * (
                    k / num_segments
                )
                segment_end_x = x_vals[0] + (x_vals[1] - x_vals[0]) * (
                    (k + 1) / num_segments
                )
                segment_start_y = y_vals[0] + (y_vals[1] - y_vals[0]) * (
                    k / num_segments
                )
                segment_end_y = y_vals[0] + (y_vals[1] - y_vals[0]) * (
                    (k + 1) / num_segments
                )
                ...
                if not np.isnan(segment_start_y) and not np.isnan(segment_end_y):
                    ax.plot(
                        ...
                        color=lc(k / num_segments),
                        linewidth=2,
                    )

        # Plot the points with the color gradient (only non-NaN distances)
        ax.scatter(  # sc = ax.scatter(
            day_data["hours_elapsed"][~day_data[distance_col].isna()],
        ...
        ax.set_yticks(
            np.arange(0, data[distance_col].max() + 1, 1)
        ...
        ax.set_xticklabels(
            [str(int(hour)) for hour in np.arange(0, 25, 1)], fontsize=10, color="gray"
        )
