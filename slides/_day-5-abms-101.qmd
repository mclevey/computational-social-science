...
:::: {.columns}
::: {.column width="55%"}
![This is the general point I want to make on this slide, come back later.](media/compare_agent_networks.png){width=100% #fig-compare_agent_networks}
:::
...
- present this a different way if I don't do the 6 models manually?
...
- emphasize heterogeneity by directly represent individual agents and their iteractions
- focuses on emergent outcomes and adaptive. comntextuialized behaviours
...
::: {.column width="45%"}
![Like generative network analysis, the high-level objective is to deepen our understanding of the latent processes governing the system.](media/flocking.jpg){width=100% .shadow-img}
:::
::::
...
:::: {.columns}
::: {.column width="45%"}
These **real-world patterns** can take many forms, such as spatial distributions, temporal sequences, or system-level properties. We want to observe and document patterns from empirical data, prior research, etc. Use these patterns to guide the design of the model structure. Then we select and organize entities, processes, and interactions in such a way tha the model is capable of generating the observed patterns. In the **SIR model example**,

- the key **patterns** are observed epidemic behaviors, such as the rise and fall of infected individuals over time, especially the occurrence of an infection peak.
- these are **incorporated into the SIR model** via the transmission rate $\beta$ [(to simulate the infection spread)]{.nord-light} and recovery rate $\gamma$ [(to simulate recovery)]{.nord-light}.
- if the model can't reproduce the observed epidemic behaviours, we **refine** it.
:::
...
In the **SIR model example**, patterns like the peak of infections or the eventual stabilization of the population into susceptible and recovered groups are used to test and refine theories of disease spread. Theories about how individuals become infected (e.g., transmission rate) and recover (e.g., recovery rate) are tested by checking whether the model can replicate these observed patterns. If the model consistently reproduces these patterns, it supports the validity of the theory. If not, the theory needs to be adjusted—perhaps by considering factors like varying contact rates or the impact of interventions, and the process is repeated until the model behaves as expected.
:::
