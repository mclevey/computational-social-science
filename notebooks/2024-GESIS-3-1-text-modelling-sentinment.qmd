...
4. Understand the Basics of Supervised Sentiment Analysis
    - Train a supervised sentiment analysis model using annotated data.
    - Evaluate the performance of a machine learning model on sentiment prediction tasks.
...
# BONUS CONTENT: SUPERVISED LEARNING

In this section, we’ll explore supervised learning for sentiment analysis using annotated data. We'll train a logistic regression model on the IMDb Movie Review dataset. This is the same data Johannes' used in his lecture content on supervised learning for text analysis.

## Loading the IMDb Dataset

We'll start by downloading the IMDb dataset from HuggingFace.

The code block below will download the IMDB Movie Review dataset from HuggingFace.
...
We'll convert the text reviews into numerical features using TF-IDF.
...
Let's train a logistic regression model to classify the sentiment of the reviews.
