...
def df_to_md(df, filepath):
    ...
    with open(filepath, 'w') as f:
        f.write(md)
df_to_md(vertex_data.head(10), 'output/vertex_data_1.md')
```
...
The MDL score is much higher! However, *this is to be expected*, as the jump from a binary to a weighted network requires a huge increase in bits to adequetely compress the network. We'll see how this model compares to the other weighted models we'll fit below.
...
Once the code block above finishes running, we can see how much we improved on the estimate from Model 2. Let's update our MDL comparison.
...
df_to_md(vertex_data.head(10), 'output/vertex_data_M1-M4.md')
```
